...
export default {
  ...
  created() {
    // manual settings
    this.$store.commit('settings/manualSettings', this.settings);

    // initiate Axios
    this.$store.commit('settings/initAxiosSettings');
  ...
  methods: {
    ...
    requestInterceptor() {
      HTTP.interceptors.request.use((config) => {
        ...
        config.baseURL = this.$store.getters['settings/baseUrl'];
        config.headers = this.$store.getters['settings/headers'];

        // loading spinner +
        this.$store.commit('messages/addLoading');

        return config;
      }, (error) => {
        // loading spinner -
        this.$store.commit('messages/subtractLoading');
      ...
      HTTP.interceptors.response.use((response) => {
        ...
        if (Object.prototype.hasOwnProperty.call(response.data, 'result')) {
          if (response.data.result.message) {
            ...
            // set action result
            this.$store.commit('messages/setActionResult', message);
          }
        }
        ...
        // set error message
        this.$store.commit('messages/setError', errorMessage);
    ...
    selectManager(managerName) {
      if (this.activeManager !== managerName) {
        this.$store.commit('setActiveManager', managerName);
      }
    },
  },
};

  ...
  methods: {
    ...
    validateDirName() {
      if (this.directoryName) {
        this.directoryExist = this.$store.getters[`${this.activeManager}/directoryExist`](this.directoryName);
      } else {
     ...
     */
    unpackArchive() {
      this.$store.dispatch('unzip', this.createFolder ? this.directoryName : null).then(() => {
        // close modal window
        this.hideModal();
      });
    },

...
export default {
  ...
  computed: {
    ...
    cmOptions() {
      return {
        mode: this.$store.state.settings.textExtensions[this.selectedItem.extension],
        theme: 'blackboard',
        lineNumbers: true,
        line: true,
      };
    ...
    editorHeight() {
      if (this.$store.state.modal.modalBlockHeight) {
        return this.$store.state.modal.modalBlockHeight - 200;
      }
  ...
  methods: {
    ...
    updateFile() {
      ...
      this.$store.dispatch('updateFile', formData).then((response) => {

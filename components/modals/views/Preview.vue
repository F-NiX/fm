...
export default {
  ...
  computed: {
     ...
     */
    auth() {
      return this.$store.getters['settings/authHeader'];
    },
     ...
     */
    selectedItem() {
      return this.$store.getters['selectedItems'][0];
    },
     ...
     */
    maxHeight() {
      if (this.$store.state.modal.modalBlockHeight) {
        return this.$store.state.modal.modalBlockHeight - 170;
      }
  ...
  methods: {
     ...
     */
    canCrop(extension) {
      return this.$store.state.settings.cropExtensions.includes(extension.toLowerCase());
    },
    ...
    loadImage() {
      ...
      if (this.auth) {
        ...
        this.imgSrc = `${this.$store.getters['settings/baseUrl']}preview?disk=${this.selectedDisk}&path=${encodeURIComponent(this.selectedItem.path)}&v=${this.selectedItem.timestamp}`;
      }
    },
  },
};

...
export default {
  ...
  computed: {
     ...
     */
    selectedItem() {
      return this.$store.getters[`${this.activeManager}/selectedList`][0];
    },
    ...
    validateName() {
      if (this.name !== this.selectedItem.basename) {
        ...
        if (this.selectedItem.type === 'dir') {
          ...
          this.directoryExist = this.$store.getters[`${this.activeManager}/directoryExist`](this.name);
        } else {
          // check file name matches
          this.fileExist = this.$store.getters[`${this.activeManager}/fileExist`](this.name);
        }
      }
    },

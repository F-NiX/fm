...
export default {
  ...
  computed: {
     ...
     */
    backDisabled() {
      return !this.$store.state[this.activeManager].historyPointer;
    },
    ...
    forwardDisabled() {
      return this.$store.state[this.activeManager].historyPointer ===
          this.$store.state[this.activeManager].history.length - 1;
    },
     ...
     */
    isAnyItemSelected() {
      return this.$store.state[this.activeManager].selected.files.length > 0 ||
          this.$store.state[this.activeManager].selected.directories.length > 0;
    },
     ...
     */
    viewType() {
      return this.$store.state[this.activeManager].viewType;
    },
     ...
     */
    uploading() {
      return this.$store.state.messages.actionProgress > 0;
    },
     ...
     */
    clipboardType() {
      return this.$store.state.clipboard.type;
    },
     ...
     */
    hiddenFiles() {
      return this.$store.state.settings.hiddenFiles;
    },
  },
  methods: {
    /**
     ...
     */
    historyBack() {
      this.$store.dispatch(`${this.activeManager}/historyBack`);
    },
     ...
     */
    historyForward() {
      this.$store.dispatch(`${this.activeManager}/historyForward`);
    },
     ...
     */
    toClipboard(type) {
      this.$store.dispatch('toClipboard', type);
     ...
     */
    toggleHidden() {
      this.$store.commit('settings/toggleHiddenFiles');
    },
     ...
     */
    selectView(type) {
      if (this.viewType !== type) this.$store.commit(`${this.activeManager}/setView`, type);
    },
    ...
    screenToggle() {
      ...
      this.$store.commit('screenToggle');
    },
  },
};
</script>

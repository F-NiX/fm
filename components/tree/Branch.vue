...
export default {
  ...
  computed: {
     ...
     */
    subDirectories() {
      return this.$store.getters['tree/directories'].filter(item => item.parentId === this.parentId);
    },
  },
  methods: {
    /**
    ...
    isDirectorySelected(path) {
      return this.$store.state.left.selectedDirectory === path;
    },
    ...
    showSubdirectories(path, showState) {
      if (showState) {
        // hide
        this.$store.dispatch('tree/hideSubdirectories', path);
      } else {
        // show
        this.$store.dispatch('tree/showSubdirectories', path);
      }
    },
    ...
    selectDirectory(path) {
      ...
      if (!this.isDirectorySelected(path)) {
        this.$store.dispatch('left/selectDirectory', { path, history: true });
      }
    },
  },
};
